// implements smart spindle controller with PID and auto tune.
...
uint16_t pwm_map(float pwm) {
    return PWM_MIN + (uint16_t)(pwm * ((PWM_MAX - PWM_MIN) / 255.0));
}
...
int main() {
    ...
    if (isnan(Kp) || isnan(Ki) || Kp < 0 || Ki < 0 || Kp > 100 || Ki > 100) {
        Kp = 0.02; Ki = 0.1;
    }

    uart_println("Spindle PID Controller (Klipper) Ready.");
    ...
    while (1) {
        ...
        if (!(PIND & (1 << BUTTON_PIN)) && (now - lastButtonPress > 500)) {
            lastButtonPress = now;
            uart_println("Relay auto-tune triggered...");
            relayAutoTune()
        }
        ...
        if (now - lastRPMNonZero > RPM_CHECK_TIMEOUT && pwmOutput > 0) {
            pwmOutput = 0;
            pwm_write(pwm_map(pwmOutput));
            uart_println("Safety Stop: RPM lost!");
        }
